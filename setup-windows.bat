...
if exist "%ProgramData%\chocolatey\bin\choco.exe" (
    ...
    echo ❌ Chocolatey is required but not found
...
if not "%SETUP_ADDITIONAL_CONF%"=="" (
    echo ⚠️ SETUP_ADDITIONAL_CONF is not supported on Windows and will be ignored
)
...
if %errorlevel%==0 (
    ...
    if !errorlevel! neq 0 (
        echo ❌ Failed to install MariaDB via Chocolatey
        exit /b 1
    )
...
if %errorlevel%==0 (
    ...
    echo ⚠️ MariaDB service may already be running or failed to start
)
...
if not "%MARIADB_DATABASE%"=="" (
    ...
    if !errorlevel!==0 (
        ...
        echo ⚠️ Failed to create database '%MARIADB_DATABASE%'
    )
)
...
if not "%SETUP_CONF_SCRIPT_FOLDER%"=="" (
    if exist "%SETUP_CONF_SCRIPT_FOLDER%" (
        ...
        if "!MY_INI_PATH!"=="" (
            echo ❌ MariaDB configuration file (my.ini) not found in any expected location
        ...
        if !CONFIG_UPDATED!==1 (
            ...
            echo ⚠️ No .cnf files found in %SETUP_CONF_SCRIPT_FOLDER%
        )
    ) else (
        echo ⚠️ Configuration script folder %SETUP_CONF_SCRIPT_FOLDER% does not exist
    )
)
